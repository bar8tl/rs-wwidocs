...
pub fn upld_idocdefn(parm: ParameTp, mut stg: SettingsTp) {
  ...
  if s.mitm { init_upldmitm(&mut ui                 ); }
  if s.sgrp { init_upldsgrp(&mut ug, GRP.to_string()); }
  if s.ssgm { init_upldssgm(&mut us, SGM.to_string()); }
  ...
  for wline in rdr.lines() {
    ...
    let line  = wline.trim();
    if line.len() > 0 {
      let sline = scan_inpline(line.to_string());
      if s.mitm { get_mitmdata(&sline, &mut ui); }
      if s.sgrp { get_sgrpdata(&cnn, &sline, &mut ug); }
      if s.ssgm { get_ssgmdata(&cnn, &sline, &mut us); }
    }
  }
